...
This is the mean-rollout-aggregation variant of analysis/ehull_rdos_u_only_study/:
same study (same starting material, same gamma=0.0001, same MCTS hyperparameters),
but the underlying MCTS run used --rollout-aggregation mean (see run_study.sh)
instead of the default 'max'. Nothing in this script itself differs from the
calibrated study's copy - gamma/composite-score computation is identical - only
the upstream MCTS run (and the starting_material_sweep_mean_rollout data used by
convergence_by_starting_material.png) used 'mean' instead of 'max'.
...
- convergence_by_starting_material.png (reuses sweep_starting_material.py's own
  starting_material_sweep_mean_rollout data here, NOT the calibrated study's
...
def plot_convergence_by_starting_material(mcts_materials_root: Path, out_dir: Path):
    ...
    Reuses sensitivity_studies/results/starting_material_sweep_mean_rollout/convergence_data.csv
    (generated by this directory's own sweep_starting_material.py, which adds
    rollout_aggregation='mean' to every replicate - the calibrated study's
    starting_material_sweep/ uses the default 'max' and is NOT reused here)
    instead of re-running MCTS inline: that sweep uses this study's exact
    composite reward (rollout_method='ehull_rdos', beta=1.0, gamma=0.0001,
    rollout_aggregation='mean' - see sensitivity_studies/scripts/common.py
    BASELINE), varying only transition_metal/group_iv, with 5 seeds per
    starting material. Each line below is the mean best-composite-so-far
    across those 5 seeds, with a shaded 10th-90th percentile band (not mean
    +/- std: std is a parametric estimate that can extend past the best value
    any seed actually reached, which would misleadingly suggest the search
    found something better than it did. A percentile band is computed
    directly from the observed seeds, so it can never exceed what was
    actually seen).
    ...
    compute_global_u_only_ranks - gamma is unchanged from the calibrated
    study, so this is still UZr6Pb6 and the same d=2/4/6/8 ladder applies),
    to show that starting further away from the optimum is *why* that
    material converges slower.
    """
    sweep_csv = (mcts_materials_root / 'sensitivity_studies' / 'results'
                 / 'starting_material_sweep_mean_rollout' / 'convergence_data.csv')
