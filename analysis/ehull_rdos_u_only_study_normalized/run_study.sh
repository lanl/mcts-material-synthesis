#!/bin/bash
# Gamma-normalized variant of analysis/ehull_rdos_u_only_study/: same study,
# same starting material/hyperparameters, but gamma is fixed to
# 1/(max raw r_DOS across the 108 U-only compounds) = 1/2516.1664410449775
# = 0.00039742998860786596, instead of the calibrated 0.0001, so the best
# compound's gamma*r_DOS term tops out at 1.0 (same scale as ehull_reward's
# ~[-1,1] range). Passed explicitly via --gamma below (CLI flags override
# config.json - see mcts_crystal/cli.py) so config.json's gamma=0.0001 stays
# untouched for the calibrated study.
...
echo "=================================================="
echo "EHULL + RDOS STUDY (U-only, gamma-normalized)"
